...
struct HourlyScheduleView: View {
    ...
    private let calendar = Calendar.current
    private let timeHeight: CGFloat = 65 // Slightly taller for breathing room
        ...
        for item in sorted {
            ...
            for (i, column) in columns.enumerated() {
                if let last = column.last,
                   item.dueDate >= (last.endDate ?? last.dueDate.addingTimeInterval(3600)) {
            ...
            ScrollView {
                ZStack(alignment: .topLeading) {
                    ...
                    VStack(spacing: 0) {
                        ForEach(hours, id: \.self) { hour in
                            HStack(alignment: .top) {
                                ...
                                Rectangle()
                                    .fill(Color(.separator)) // Standard separator color
                                    .frame(height: 1)
                            }
                    ...
                    GeometryReader { geo in
                        let availableWidth = geo.size.width - 70
                        
                        ForEach(timedItems) { item in
                            let width = (availableWidth / CGFloat(item.colSpan)) - 4
                            let x = 60 + (CGFloat(item.colIndex) * width) + (CGFloat(item.colIndex) * 4)
                            
                            TaskBubble(deadline: item.deadline, onToggle: onToggle)
                                .frame(width: width, height: item.height)
                                .position(x: x + width/2, y: item.yOffset + item.height/2 + 20)
                        }
                    }
                    .frame(height: CGFloat(hours.count) * timeHeight + 50)
                }
                .padding(.vertical)
            }
        }
    ...
    func calculateGeometry(for item: Deadline) -> (y: CGFloat, h: CGFloat)? {
        ...
        let height = CGFloat(duration / 3600.0) * timeHeight
        
        return (y: startOffset, h: max(height, 40)) // Min height 40
    }
}
